...
export function EnhancedKeywordResults({
  analysisId,
  websiteName,
  summaryOnly = false,
  keywordsOnly = false,
  ...
  const competitionDistribution = useMemo(() => {
    if (!data?.rankedKeywords && !data?.keywordIdeas) return []
  ...
  // Calcul des opportunités (même logique que keyword-results.tsx)
  const opportunityCount = useMemo(() => {
    if (!data?.keywordIdeas) return 0

    const opportunities = data.keywordIdeas.filter(
      (k) => k.searchVolume > 500 && k.difficulty < 60
    )
  ...
  // Affichage conditionnel selon les props
  if (summaryOnly) {
    // Seulement le dashboard héro
    return (
      <>
  ...
  if (keywordsOnly) {
    // Seulement les cartes de mots-clés sans titre ni sections
    return (
      <>
        <div className="grid gap-4">
          {[...data.rankedKeywords, ...data.keywordIdeas]
            .slice(0, 30)
            .map((keyword) => (
              <Card
                key={keyword.keyword}
  ...
  if (pagesOnly) {
    // Vue d'analyse par pages (utilise le composant GroupedKeywordResults)
    return (
      <>
  ...
  // Vue complète avec tous les tabs (pas utilisée dans la nouvelle architecture)
  return (
    <div className="space-y-6">
      {/* Hero Dashboard */}
      <Card className="border-violet-200 bg-gradient-to-r from-violet-50 to-purple-50 dark:border-violet-800 dark:from-violet-950/30 dark:to-purple-950/30">
        <CardHeader>
          ...
          <CardDescription className="text-gray-600 dark:text-gray-300">
            Données enrichies de{" "}
            {data.rankedKeywords.length + data.keywordIdeas.length} mots-clés
            avec insights concurrentiels • {opportunityCount} opportunités
            identifiées
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-4 text-center dark:border-gray-700 dark:bg-gray-800">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {data.keywordIdeas.length > 0
                    ? formatCurrency(
                        data.keywordIdeas.reduce((sum, kw) => sum + kw.cpc, 0) /
                          data.keywordIdeas.length,
                        "CA"
                      )
                    : formatCurrency(0, "CA")}
      ...
      {/* Vue d'ensemble uniquement - graphiques et analytics */}
