  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        avgCpc:
          Math.round(
            (processedKeywords.reduce((sum: number, k: any) => sum + k.cpc, 0) /
              processedKeywords.length) *
              100
          ) / 100 || 0,
        avgPosition:
          Math.round(
            processedKeywords
              .filter((k: any) => k.currentPosition)
              .reduce(
                (sum: number, k: any, _: any, arr: any[]) =>
                  sum + (k.currentPosition || 0) / arr.length,
                0
              )
          ) || 0,
        totalEtv:
          Math.round(
            processedKeywords.reduce((sum: number, k: any) => sum + k.etv, 0) *
              100
          ) / 100,
        totalPaidValue:
          Math.round(
            processedKeywords.reduce(
              (sum: number, k: any) => sum + k.estimatedPaidCost,
              0
            ) * 100
          ) / 100,
      },
